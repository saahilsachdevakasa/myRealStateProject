...
## E14 gotchas (validation rules, CP base infrastructure)

43. **`ValidationRule.description` has the same 255-character limit as `PermissionSet.description`.** Salesforce rejects with `Validation rule description cannot be longer than 255 characters long. (line:col)` — same root cause as gotcha #18, broader scope than originally documented. The 255-char cap applies to *most* short-form description fields across declarative metadata types (validation rules, permission sets, queues, page layouts) — treat any `<description>` block in declarative metadata as tooltip-length, not paragraph-length, unless the type's WSDL explicitly says otherwise (custom object/field descriptions are 1000-char, the exception). Audit before deploy: `for f in path/to/*.xml; do desc=$(grep '<description>' "$f" | sed 's/.*<description>\(.*\)<\/description>.*/\1/'); echo "$(basename $f): $(echo -n "$desc" | wc -c) chars"; done`. Salesforce's column number in the error points to where the description tag closes, not where the overflow starts — count chars from the opening tag.

## E18 gotchas (Agentforce metadata retrieval / CLI quirks)

45. **`sf project retrieve start` in source format can throw a false-positive `TypeInferenceError: Unexpected child metadata ... found for parent type [CustomObject]` on the first QuickAction file (alphabetically) in any object's `quickActions/` folder.** Confirmed by removing the flagged file and re-running: the error simply moved to the new alphabetically-first sibling. The file itself is valid — this is a genuine unfixed bug in `@salesforce/cli`'s bundled `source-deploy-retrieve` library, **reproduced identically on both 2.120.3 and the latest 2.147.7** — upgrading the CLI does NOT fix it, so don't burn time on a CLI update expecting this to go away. It affects **any** manifest-based source-format retrieve project-wide, not just one component. `RealEstateSDO` also reports `NonSourceTrackedOrgError` on `sf project reset tracking` — it is not a source-tracked org type, so local shadow-git tracking is not the cause either. **Workaround** (does not touch the org): retrieve in **metadata format** instead — `sf project retrieve start --manifest <file> --target-org <alias> --target-metadata-dir <dir> --unzip` — which bypasses the buggy local source-tree resolution entirely. Then hand-place the result into `force-app/`: for simple types (e.g. `ApexClass`) the metadata-format files are already source-format-identical, just copy them. For decomposed bundle types (e.g. `GenAiFunction`, `GenAiPlannerBundle`) rename the bare `<Name>.<type>` file to `<Name>.<type>-meta.xml` and copy subfolders (`input/`, `output/`, `localActions/`, etc.) unchanged. Validate the reconstruction with `sf project deploy start --source-dir <paths> --target-org <alias> --dry-run --test-level NoTestRun` before trusting it — a clean `Unchanged` state on every file confirms the hand-placement is byte-correct.

46. **Newer Agentforce metadata types require a higher API version than the rest of the org's Apex.** `GenAiFunction`/`GenAiPlugin` retrieved cleanly at API version 62.0 (matching the org's Apex classes), but `GenAiPlannerBundle` failed at both 62.0 (`Entity type 'GenAiPlannerBundle' is not available in this api version`) and 64.0 (`UNSUPPORTED_API_VERSION: ... update to package version v65 or higher`) — it only succeeded at **65.0**. When retrieving Agentforce metadata, don't assume one `<version>` in the manifest covers every Agentforce-related type; bump it per-type until the entity-not-available error clears. `GenAiPlugin` (topics) has no separate retrievable file footprint of its own in metadata-format retrieval — its content is fully embedded inside the owning `GenAiPlannerBundle`'s `<localTopics>` elements, so a manifest entry for `GenAiPlugin` alongside `GenAiPlannerBundle` returns nothing extra and is not an error.
